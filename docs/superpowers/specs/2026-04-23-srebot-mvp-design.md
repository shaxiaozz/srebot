...
| 配置格式 | 仅 `config.json`(JSON),通过 `--config <path>` 指定;api_key 明文,文件权限 0600;保留 `agents.defaults.*` 嵌套对齐 nanobot |
...
**配置加载:** CLI 必须传 `--config <path>` 指定 `config.json`;文件内缺失字段由代码中的 `Defaults()` 填充。不支持 env 插值、不支持 YAML、不自动寻址默认路径。
...
配置根为 `config.json`,结构对齐 nanobot 的 `agents.defaults.*` 嵌套,MVP 只保留用到的字段:
...
// MCPServer 支持两种 transport:
//   - HTTP: 设置 URL(优先)
//   - stdio: 设置 Command + Args (+ Env)
// 二者互斥;URL 非空则走 HTTP。
type MCPServer struct {
    ...
    ToolTimeout int               `json:"toolTimeout,omitempty"` // 秒,默认 60
}
...
// AgentConfig 是单个 agent 的 LLM / 工作区参数(允许 per-agent 覆盖)。
...
// Resolved 是合并 Root 后供运行时使用的平面结构。
...
func Load(path string) (*Resolved, error)   // 读 JSON → 填 Defaults → Validate → 返回 Resolved
func Defaults() Root
func (r *Resolved) Validate() error
```

**设计要点:**
- `agents.defaults.*` 只管 LLM + 工作区参数(可 per-agent 覆盖,MVP 只用 defaults)
- `approval` / `logging` / `memory` 是**全局运行时配置**,顶层同级,不随 agent 切换
- `Load` 返回 `Resolved` 平面结构,下游组件不感知 `agents.defaults.*` 嵌套

**安全:** `config.json` 含明文 `apiKey`,加载前检查文件权限不得宽于 `0600`(Unix),否则 warn。用户需自行管理该文件。

**CLI flag:** 只保留运行时行为开关,不做"配置覆盖":
- `--config <path>`(必需)
- `--resume`
- `--yolo`(覆盖 `approval.yolo`,仅此一处例外)
- `--verbose`(覆盖 `logging.level`,仅此一处例外)
- `--session-id <id>`
...
**Transport:** 两种并存,按 `MCPServer` 字段自动区分:
- `url` 非空 → HTTP transport(Streamable HTTP / SSE,由 mcp-go 决定)
- 否则走 stdio,需 `command` + `args`(+ 可选 `env`)
- `toolTimeout`(秒)作用于单次 `CallTool`,缺省 60

**配置位置:** 顶层 `mcpServers`(与 `agents` / `approval` 等同级),不再使用独立 `mcp.yaml`。这样 `${VAR}` 插值可在 config 层统一处理(spec R3)。
...
- 默认 INFO,`--verbose` 开 DEBUG,使用 Go 标准库 `slog`(Go 1.25.8)
